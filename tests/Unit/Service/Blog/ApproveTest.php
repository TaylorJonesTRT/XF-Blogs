...
use TaylorJ\Blogs\Service\Blog\Approve;
...
class ApproveTest extends TestCase
{
    ...
    protected function createService(array $blogValues = [])
    {
        ...
        return new Approve($this->app(), $blog);
    }

    public function testConstructorAcceptsBlog()
    {
        $blog = $this->makeBlog();
        $service = new Approve($this->app(), $blog);

        $this->assertInstanceOf(Approve::class, $service);
    ...
    {
        $reflection = new \ReflectionClass(Approve::class);
    ...
    {
        $service = $this->createService();
        $this->assertInstanceOf(Blog::class, $service->blog);
    }
}
